...
def test_serial_port_and_baud_flags_parse():
    args = parse_cli_args(["--port", "COM7", "--baud", "57600", "--serial-timeout", "1.5"])
    assert args.port == "COM7"
    assert args.baud == 57600
    assert args.serial_timeout == 1.5


def test_invalid_serial_timeout_rejected():
    with pytest.raises(SystemExit):
        parse_cli_args(["--serial-timeout", "0"])
